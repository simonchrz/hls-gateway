...
def api_internal_training_snapshot():
    ...
    for d in sorted(HLS_DIR.glob("_rec_*")):
        ...
        try:
            ...
            pass
        # ads.json is a transient lazy-regenerated cache. After
        # cutlist-upload (Mac-daemon delivers fresh detect result) the
        # .txt is written but ads.json is NOT regenerated until next
        # /recording/<uuid>/ads call. Snapshot used to miss those
        # recordings entirely (= count drops 177→137 across 1 day after
        # yesterday's cutlist-upload bug fixes processed retries). Fall
        # back to comskip-parse of the cutlist .txt directly when
        # ads.json is empty/missing — same parser /recording/<uuid>/ads
        # already uses for the same reason.
        if not ads_auto:
            try:
                blocks = _rec_parse_comskip(d) or []
                if blocks:
                    ads_auto = [[float(s), float(e)] for s, e in blocks]
            except Exception:
                pass
        pseudo = None
        try:
...
def api_internal_auto_confirm_bulk():
    ...
    for d in HLS_DIR.glob("_rec_*"):
        ...
        out[uuid] = {
            ...
            "has_whisper": bool(v.get("has_whisper")),
            "has_anchors": bool(v.get("has_anchors")),
            "reason": v.get("reason", ""),
        }
...
@app.route("/api/internal/detect-give-up/<uuid>", methods=["POST"])
def api_internal_detect_give_up(uuid):
    """Mac daemon calls this after exhausting MAX_DETECT_RETRIES on a
    recording (= tv-detect rc!=0 N times in a row). Removes BOTH
    .detect-requested and .detect-requested-low markers so the
    recording stops blocking the queue. The recording stays as-is
    (= prior cutlist if any survives; otherwise empty .txt). User
    can manually /redetect later to re-arm.

    Without this endpoint a single broken recording (= rtlzwei
    + bottom-positioned logo + cropdetect false letterbox →
    NewLogoDetector pre-clamping crash) stalled the entire
    re-detect queue, because the daemon's _detect_low fetch is
    gated on _detect being empty (line 956 of tv-thumbs-daemon)."""
    rec = HLS_DIR / f"_rec_{uuid}"
    if not rec.is_dir():
        return Response(json.dumps({"ok": False, "error": "unknown uuid"}),
                        status=404, mimetype="application/json")
    cleared = []
    for marker_name in (".detect-requested", ".detect-requested-low"):
        m = rec / marker_name
        if m.exists():
            try:
                m.unlink()
                ...
                print(f"[detect-give-up] {uuid[:8]} unlink {marker_name} "
                      f"err: {e}", flush=True)
    print(f"[detect-give-up] {uuid[:8]} cleared {cleared} "
          f"(daemon exhausted retries)", flush=True)
    return _cors(Response(json.dumps({"ok": True, "cleared": cleared}),
                          mimetype="application/json"))
...
def api_internal_detect_config(uuid):
    ...
    try:
        ...
        # Logo CNN opt-in per channel. Channels where the edge-template
        # works well (= kabel-eins/sat-1/prosieben hitting 22%+ blocks
        # in production) keep it; channels where edge-template fails
        # (= VOX/Nick/RTL/RTLZWEI/sixx/comedy-central with 3-12% rates
        # due to busy ad-content false-positives) opt into the CNN
        # via "logo_cnn": true. CNN ONNX must exist Pi-side at
        # /mnt/tv/hls/.tvd-logo-cnn/<slug>.logo-cnn.onnx — silent
        # fallback to edge-template if missing.
        if (ch_cfg.get("logo_cnn") and slug
                and (_TVD_LOGO_CNN_DIR / f"{slug}.logo-cnn.onnx").is_file()):
            cached_logo_cnn_url = f"/api/internal/detect-logo-cnn/{slug}"
    except Exception: pass
    ...
    #
    # Same file also carries optional `logo_smooth_s` per show. Channel
    # default is 0 for most channels, but specific shows benefit from
    # smoothing when the NN logo signal under-fires at ad-start (= snap
    # window then misses the bumper peak by >90 s on shows like Ulrich
    # Wetzel — A/B on n=6 RTL recordings showed channel-wide smooth=5
    # was a net wash but UW alone gained +0.136 IoU).
    if show_title:
        try:
            ...
            if show_learned:
                # Only override drift values if the show entry has them
                # (a {logo_smooth_s: 5}-only entry must not reset the
                # per-channel start_lag/end_shrink learned drift to 0).
                drift_keys = ("start_lag", "start_shrink",
                              "sponsor_duration", "end_shrink")
                if any(k in show_learned for k in drift_keys):
                    start_ext, end_ext = _signed_drift(show_learned)
                if "logo_smooth_s" in show_learned:
                    smooth_s = float(show_learned["logo_smooth_s"])
        except Exception:
            pass
    ...
    try:
        ...
        pass
    # Recording start_ts + test-set membership: lets daemon decide
    # whether to invoke whisper-postprocess (= expensive but only matters
    # for recent + test-set recordings per Smart-V2 logic).
    start_ts = 0
    is_test_uuid = False
    try:
        dvr = json.loads(urllib.request.urlopen(
        ...
        for e in dvr.get("entries", []):
            if e.get("uuid") == uuid:
                start_ts = e.get("start_real") or e.get("start") or 0
                break
    except Exception:
        pass
    try:
        ts_path = HLS_DIR / ".tvd-models" / "head.test-set.json"
        if ts_path.is_file():
            is_test_uuid = uuid in json.loads(
                ts_path.read_text()).get("uuids", [])
    except Exception:
        pass
    return _cors(Response(json.dumps({
        "uuid": uuid,
        ...
        "cached_logo_cnn_url": cached_logo_cnn_url,
...
@app.route("/api/internal/detect-logo-cnn/<slug>")
def api_internal_detect_logo_cnn(slug):
    """Serve the per-channel ONNX logo classifier (= scripts/
    train_logo_cnn.py output). Replaces the edge-template Sobel
    signal in tv-detect for channels where this file exists; daemon
    decides per-recording based on detect-config's cached_logo_cnn_url
    field. ~1 MB inline-weights ONNX, mimetype octet-stream so curl
    doesn't try to render."""
    p = _TVD_LOGO_CNN_DIR / f"{slug}.logo-cnn.onnx"
    if not p.is_file():
        abort(404)
...
def api_internal_detect_logo(slug):
    # ?res=hd → serve the HD-trained logo bbox (= different pixel coords
    # because HD frames are 1920x1080 vs SD's 720x576). Falls back to
    # the SD logo if no HD-trained file exists, so the daemon's HD-aware
    # request stays usable while we incrementally train HD logos
    # per-channel. Without the fallback, every freshly-HD-routed
    # channel without an HD logo yet would 404 and detect would lose
    # the logo signal entirely (worse than the scaled-but-wrong-coord
    # bbox — at least the SD bbox is in the right corner of the frame
    # and nearest-neighbor scaling keeps the relative position).
    res = (request.args.get("res") or "").lower()
    if res == "hd":
        hd_p = _TVD_LOGO_DIR / f"{slug}.logo.hd.txt"
        if hd_p.is_file():
            return send_file(hd_p, mimetype="text/plain")
...
def api_bumper_delete(slug, fname):
    ...
    try:
        ...
        return Response(json.dumps({"ok": False, "error": str(e)}),
                        status=500, mimetype="application/json")
    # No exclude_uuid here: bumper DELETE has no "recording we're
    # capturing from" context (unlike bumper-capture). Invalidate
    # the entire channel so the deleted template doesn't keep
    # firing in cached cutlists.
...
def api_internal_detect_models(fname):
    ...
    allowed = {
        ...
        "head.calibration.json", "head.channel-map.json",
    }
...
def api_internal_cutlist_uploaded(uuid):
    ...
    if not rec_dir.exists():
        abort(404)
    # Derive the cutlist output filename. Three cases:
    #   (a) original .ts on Pi → use its stem
    #   (b) Pi-original gone, .source-recovered.ts present → derive from
    #       existing show-name .txt in rec_dir, else tvh's recorded filename
    #   (c) Pi-original gone AND no .source-recovered.ts → daemon ran
    #       detect on its local T7-cached .ts copy, gateway has no path
    #       to give. Still write the cutlist: derive from rec_dir's
    #       existing .txt (= the 0-byte invalidation marker train-head
    #       leaves behind) or tvh's recorded filename.
    src = _rec_source_path(uuid)
    base = None
    if src and Path(src).name != ".source-recovered.ts":
        base = Path(src).stem  # (a)
    if base is None:
        # (b) or (c) — same fallback chain
        candidate = next((t for t in rec_dir.glob("*.txt")
                          if not t.name.startswith(".")
                          and not any(t.name.endswith(s) for s in
                                      (".logo.txt", ".cskp.txt",
        ...
        if candidate:
            ...
            try:
                ...
                base = Path(fn).stem if fn else None
            except Exception:
                base = None
    if not base:
        abort(404)
